...
static void dma_schedule_next_transfer(void)
{
...
static void dma_handle_next_transfer(u32 channel, s32 ticks_late)
{
    ...
    else
    {
        ...
        // since some games interrupt an MDECin transfer with an MDECout one, we need to also handle channel switching here
        dma_schedule_next_transfer();
    }
            ...
            else
            {
                ...
                if (g_dma.transfers[channel].in_progress)
                {
                    ...
                    if ((s32)channel == g_dma.active_channel)
                    {
                        SY_ASSERT(g_dma.event);
                        remove_event(g_dma.event);
                        // since the next channel to transfer relies on the current channel to re-select a channel, we need to do it here
                        dma_schedule_next_transfer();
                    }
                }
            }
